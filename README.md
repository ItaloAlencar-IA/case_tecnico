...
# h2 1. Documentação do Processo de Importação
...
# h3 1.3 Estrutura das Planilhas (colunas e tipos de dados)
...
# h3 1.4 Tratamentos aplicados (conversão de tipos, validações, normalizações)
Todas as colunas de data já formatadas para "DD/MM/YYYY" (via apache hop)
Trigger de fornecedor inserido (via apache hop)


# h3 1.5 Ajustes ou correções realizadas durante o processo
...
# h2 2. Consultas sql básicas

# h3 2.1. Total de vendas em quantidade e em valores por cada produto no mês de fevereiro de 2025
...
# h3 2.2 Pedidos requisitados mas não existe ordem de compra
...
# h3 3. Transformações de dados

# h3 3.1. Concatenar os campos produto_id e descricao_produto (onde houver) no formato;
...
# h3 3.2. Transformar o campo de datas para o formato `DD/MM/YYYY`; 

Feito para todos via Apache Hop

# h3 3.3. Retornar os dados filtrando apenas os produtos requisitados mais de 10 vezes no período.
...
# h3 3.4. Criar uma trigger que gere automaticamente um novo `idfornecedor` numérico na tabela de produtos que se relacione com a tabela de fornecedor.

Feito via Apache Hop no pipeline da tabela de fornecedor


# h3 4. Estratégia de validação com o cliente

# h3 4.1. Faria perguntas relacionadas a 3 relações diretas: 
...
# h3 4.2. Um dos métodos foi o que foi sinalizado acima resultado feito x cliente, relação entre tabelas, ETL bem consolidado (tratamento de datas, colunas que causem estranheza).
